                    ...
                    while refresh_count <= max_refresh:
                        ...
                        try:
                            ...
                            for row in rows:
                                try:
                                    # 在 td.wtitle 中找活動名稱
                                    title_elem = row.find_element(By.CSS_SELECTOR, "td.wtitle a, .td.wtitle a")
                                    activity_name = title_elem.text.strip()
                                    
                                    if keyword in activity_name:
                                        print(f"✅ 找到活動: {activity_name}")
                                        
                                        # 在同一行的 td.tdbtnw 中找按鈕
                                        # 結構：td.tdbtnw > div.ActionArea > a.btn.btn-sign
                                        btn = row.find_element(By.CSS_SELECTOR, "td.tdbtnw div.ActionArea a.btn-sign, td.tdbtnw .ActionArea .btn-sign")

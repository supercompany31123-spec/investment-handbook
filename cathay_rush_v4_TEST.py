...
# Log 檔案路徑
LOG_FILE = "/tmp/cathay_login_log_TEST.txt"
...
try:
    ...
    # 填入身份證字號
    log("填入身份證字號...")
    driver.find_element(By.ID, "ID").send_keys("F125691441")
    time.sleep(0.5)
    
    # 填入出生年月日
    log("填入出生年月日...")
    driver.find_element(By.ID, "BirthDate").send_keys("19830904")
    time.sleep(0.5)

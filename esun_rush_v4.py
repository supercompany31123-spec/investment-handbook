...
def take_screenshot():
    """使用 peekaboo 截圖"""
    import os
    os.system(f"peekaboo image --mode screen --path {SCREENSHOT_PATH} --retina")
    time.sleep(1)

def send_to_discord():
    """發送截圖到 Discord"""
    import os
    os.system(f'openclaw message send --channel discord --target 1481949086306406400 --media {SCREENSHOT_PATH}')
